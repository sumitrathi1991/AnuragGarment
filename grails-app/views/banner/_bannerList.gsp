	...
	<!-- Table heading -->
		...
		<g:each in="${bannerList}" var='banner'>
			...
			<tr class="gradeX">
				...
				<td>
					<img id="image" src="<g:createLink action="renderImageById" controller="image" params="[imageId : "${banner.image.id}"]"/>" height="60" width="55" border="0"/>
				</td>
				<td>
					<g:if test="${banner.isPublished == false}">
						<g:remoteLink controller="banner" action="publishBanner" params="${[id: banner.id,sortType:'id']}" update="tab-BannerList">
				...
				<td>
					<i class="icon-edit"></i>
					<g:remoteLink controller="banner" action="deleteBanner" params="${[id: banner.id,sortType:'id']}" update="tab-BannerList">
